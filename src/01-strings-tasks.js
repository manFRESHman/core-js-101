...
function encodeToRot13(s) {
  function encode(c) {
    if (c.match(/[a-z]/i)) {
      ...
      return n > 90 ? ((n - 'a'.charCodeAt(0) + 13) % 26) + 'a'.charCodeAt(0) : ((n - 'A'.charCodeAt(0) + 13) % 26) + 'A'.charCodeAt(0);
    }
    return c.charCodeAt(0);
  }
  return [...s].map((c) => String.fromCharCode(encode(c))).join('');
}
